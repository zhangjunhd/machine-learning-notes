...
- [NLTK入门](nltk-introduction.ipynb)
    - 搜索文本
    - 计数词汇
    - 将文本当作词链表
    - 简单的统计
...
- [获取文本语料库](corpus.ipynb)
    ...
    - 载入你自己的语料库
        - txt文件载入
        - 载入宾州树库的副本
- [条件频率分布](conditional-frequency-distribution.ipynb)
    - 条件与事件
    - 按文体计数词汇
    - 绘制分布图和分布表
    - 使用Bigrams生成随机文本
...
- [处理原始文本](handle-with-raw-text.ipynb)
    - 从网络和硬盘访问文本
        - 电子书
        - 处理HTML
        - 处理RSS订阅
    ...
    - 规范化(`Normalizing`)文本
        ...
        - 分割(`Segmentation`)
            ...
            - 将分词转换成搜索问题
                - 评估函数
                - 模拟退火
- [分类和标注词汇](classify-and-pos-tagging.ipynb)
...
- [分类文本性别鉴定](classify_gender.ipynb)
    - naive Bayes classifier
    - 似然比(`likelihood ratios`)
- [分类文本电影评论正负面评价](classify_movie_reviews.ipynb)
    - NaiveBayesClassifier
- [分类文本词性标注](classify_pos_tagging.ipynb)
...
- [分类文本句子分割](classify_sentence_segment.ipynb)
- [识别对话行为类型](recognizing-dialog-behaviour.ipynb)
- [识别文字蕴含](recognizing-textual-entailment.ipynb)
- [决策树](decision-tree.ipynb)
    - 熵(Entropy)和信息增益(Information Gain)
- [朴素贝叶斯](naive-bayes.ipynb)
- [最大熵](maximum-entropy.ipynb)
- [从文本提取信息](extract-from-text.ipynb)
    - 信息提取
        - 信息提取结构
    - 分块(Chunking)
        - 名词短语分块
...
- [分析句子结构](sentence-structure.ipynb)
    - 文法(Syntax)的用途
    - 上下文无关文法
